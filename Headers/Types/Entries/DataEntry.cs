/**
 * Serana - Copyright (c) 2018 - 2019 r0da [r0da@protonmail.ch]
 *
 * This work is licensed under the Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License.
 * To view a copy of this license, visit http://creativecommons.org/licenses/by-nc-nd/4.0/ or send a letter to
 * Creative Commons, PO Box 1866, Mountain View, CA 94042, USA.
 *
 * By using Serana, you agree to the above license and its terms.
 *
 *      Attribution - You must give appropriate credit, provide a link to the license and indicate if changes were
 *                    made. You must do so in any reasonable manner, but not in any way that suggests the licensor
 *                    endorses you or your use.
 *
 *   Non-Commercial - You may not use the material (Serana) for commercial purposes.
 *
 *   No-Derivatives - If you remix, transform, or build upon the material (Serana), you may not distribute the
 *                    modified material. You are, however, allowed to submit the modified works back to the original
 *                    Serana project in attempt to have it added to the original project.
 *
 * You may not apply legal terms or technological measures that legally restrict others
 * from doing anything the license permits.
 *
 * No warranties are given.
 */

using System;
using System.Collections.Generic;

using Serana.Engine.Streams;

namespace Serana.Engine.Headers.Types
{
    public class DataEntry : NumericEntry
    {
        public object[] values;
        public int count;

        public DataEntry(List<Entry> list,
            bool is32bit,
            string name,
            int offset,
            int count,
            EntrySize size) : base(list, is32bit, name, offset, size)
        {
            this.count = count;
        }

        public override int getSize()
        {
            // x64 mais meme size
            if (!this.changeFor64 && !this.is32bit)
                return ((int)this.size) * this.count;

            return this.is32bit ? ((int)this.size) * this.count : ((int)this.size64) * this.count;
        }

        public override void readValue(Reader reader)
        {
            this.values = new object[this.count];

            EntrySize currentSize = getEntrySize();
            int currentOffset = getOffset();

            for (int i = 0; i < this.count; i++)
            {
                switch (currentSize)
                {
                    case EntrySize._8Bits:
                        this.values[i] = reader.readByte(currentOffset + i);
                        break;
                    case EntrySize._16Bits:
                        this.values[i] = reader.readInt16(currentOffset + (i * ((int)currentSize / 8)));
                        break;
                    case EntrySize._32Bits:
                        this.values[i] = reader.readInt32(currentOffset + (i * ((int)currentSize / 8)));
                        break;
                    case EntrySize._64Bits:
                        this.values[i] = reader.readInt64(currentOffset + (i * ((int)currentSize / 8)));
                        break;
                    default:
                        break;
                }
            }
        }

        public string getName()
        {
            return this.name;
        }

        public string ToString()
        {
            string result = "";

            for (int i = 0; i < this.values.Length; i++)
            {
                result += (char)(int)this.values[i];
            }

            return result.Replace("\x00", "");
        }

        public override byte[] export()
        {
            List<byte> result = new List<byte>();

            for (int i = 0; i < this.count; i++)
            {
                switch (this.size)
                {
                    case EntrySize._8Bits:
                        result.Add((byte)(int)this.values[i]);
                        break;
                    case EntrySize._16Bits:
                        Utils.addArrayToList<byte>(result, Utils.ToInt16(Convert.ToInt16(this.values[i])));
                        break;
                    case EntrySize._32Bits:
                        Utils.addArrayToList<byte>(result, Utils.ToInt32(Convert.ToInt32(this.values[i])));
                        break;
                    case EntrySize._64Bits:
                        // TODO 
                        Utils.addArrayToList<byte>(result, Utils.ToInt64(Convert.ToInt32(this.values[i])));
                        break;
                    default:
                        break;
                }
            }

            return result.ToArray();
        }
    }
}
